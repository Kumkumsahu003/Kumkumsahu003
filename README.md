# Hi there 👋, I'm Kumkumsahu003

## 🚀 About Me
Passionate developer exploring web technologies, and Java full-stack development. Currently building projects in HTML/CSS, Java, and JavaScript.

- 🔭 I’m currently working on improving my skills in **Web Development** and **Java**.
- 🌱 Learning **Machine Learning**, **Modern JavaScript frameworks**, and **Backend Development**.
- 👯 Looking to collaborate on interesting open-source projects.
- 📫 Reach me at: [kumkumsahu365@gmail.com] or via GitHub Issues.

---

## 🛠️ Technologies & Tools

**Frontend:**
- HTML5, CSS3, JavaScript, React, Redux
- Responsive Design

**Backend & Languages:**
- Java
- Python 

**Tools & Others:**
- Git & GitHub
- Eclips
- Vs code

---

## 🔥 Featured Projects

### [Finsweet](https://github.com/Kumkumsahu003/Finsweet)
A modern multi-page website built with clean HTML & CSS. Includes sections like Home, About, Blog, Contact, etc.

### [ai-content-generator](https://github.com/Kumkumsahu003/ai-content-generator)
AI-powered content generation tool.

### [email-classifier](https://github.com/Kumkumsahu003/email-classifier)
Machine learning project for classifying emails.

### [Amazon-clone](https://github.com/Kumkumsahu003/Amazon-clone)
Frontend clone of Amazon shopping interface.

### [ATM.java](https://github.com/Kumkumsahu003/ATM.java)
Java-based ATM simulation system.

### [Worker-Seeker-Website](https://github.com/Kumkumsahu003/Worker-Seeker-Website)
Platform connecting workers and seekers (in progress).

---

## 📊 GitHub Stats

![Kumkumsahu003's GitHub Stats](https://github-readme-stats.vercel.app/api?username=Kumkumsahu003&show_icons=true&theme=radical)

---

## 🌐 Connect with Me
- GitHub:(https://github.com/Kumkumsahu003)

---

**Thanks for visiting my profile!** ✨  
Feel free to explore my repositories and star the ones you like!
